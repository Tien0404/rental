...
class PostApi {
  final String token;
  final String baseUrl = 'http://192.168.1.99:3000/api/posts';
  PostApi(this.token);
  ...
  Future<http.Response> _sendPostRequest(Uri uri, Map<String, dynamic> postData,
      List<XFile> selectedImages, String method) async {
  ...
  Future<void> createPost(
      Map<String, dynamic> postData, List<XFile> selectedImages) async {
  ...
  Future<void> updatePost(String postId, Map<String, dynamic> updatedPostData,
      List<XFile> selectedImages) async {
    await _checkConnectivity();
    final uri = Uri.parse('$baseUrl/$postId');
    final response =
        await _sendPostRequest(uri, updatedPostData, selectedImages, 'PUT');
