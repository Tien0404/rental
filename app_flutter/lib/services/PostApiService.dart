...
class PostApiService {
  ...
  Future<List<dynamic>> getAllPosts(
      {String? title, String? location, String? categoryId}) async {
    try {
  ...
  Future<Map<String, dynamic>> getPostById(String id) async {
    try {
      final url =
          Uri.parse('$baseUrl/$id'); // Dùng ID để gọi API chi tiết bài đăng
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception(
            'Không thể lấy chi tiết bài đăng: ${response.statusCode}');
      }
    } catch (error) {
